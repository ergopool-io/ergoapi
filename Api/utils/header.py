...
class HeaderWithoutPow:
    ...
    def decode_nbits(self):
        ...
        """
        size = int(self.nBits >> 24) & 455
        bytes_ = [b'\x00'] * (size + 4)
        bytes_[3] = size.to_bytes(1, 'little')
        if size >= 1:
            bytes_[4] = ((self.nBits >> 16) & 255).to_bytes(1, 'little')
        if size >= 2:
            bytes_[5] = ((self.nBits >> 8) & 255).to_bytes(1, 'little')
        if size >= 3:
            bytes_[6] = (self.nBits & 255).to_bytes(1, 'little')
...
class Writer:
    ...
    def put_bytes_big_indian(self, x: int):
        """
...
class HeaderSerializer:
    ...
    def parse_without_pow(reader: Reader):
        ...
        n_bits = reader.get_bytes_big_indian()
    ...
    def serialize_without_pow(header: HeaderWithoutPow, writer: Writer):
        ...
        writer.put_bytes_big_indian(header.nBits)
